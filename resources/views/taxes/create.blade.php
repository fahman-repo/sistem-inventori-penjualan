...
@section('admin_title', 'Tambah Pajak')

@section('content_header', 'Tambah Pajak Baru')
                    ...
                    <form action="{{ route('taxes.store') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                                   value="{{ old('name') }}" required placeholder="Contoh: PPN 11%">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('rate') is-invalid @enderror" id="rate" name="rate"
                                   value="{{ old('rate') }}" step="0.01" min="0" max="100" required placeholder="Contoh: 11">
                            @error('rate')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
