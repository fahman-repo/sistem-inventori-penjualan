                        ...
                        <form id="purchase-form" action="{{ route('purchases.update', $purchase) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="payment_status" id="payment_cash" value="cash"
                                        {{ old('payment_status', $purchase->payment_status) == 'cash' ? 'checked' : '' }}>
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="payment_status" id="payment_credit" value="credit"
                                        {{ old('payment_status', $purchase->payment_status) == 'credit' ? 'checked' : '' }}>
                            ...
                            <div id="due-date-info" class="alert alert-info" style="display:{{ $purchase->payment_status == 'credit' ? 'block' : 'none' }};">
                                <i class="fa fa-calendar"></i> <strong>Jatuh Tempo:</strong> <span id="due-date-display">
                                    @if($purchase->supplierDebt && $purchase->supplierDebt->due_date)
                                        {{ $purchase->supplierDebt->due_date->format('d/m/Y') }}
                                    @else
                                        -
                                    @endif
                                </span>
                            ...
                            <div class="form-group">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input" id="use_tax" {{ $purchase->tax_id ? 'checked' : '' }}>
                                    <label class="custom-control-label" for="use_tax">Kena Pajak</label>
                                </div>
                            </div>

                            <div id="tax-section" style="display:{{ $purchase->tax_id ? 'block' : 'none' }};">
                                <div class="form-group">
                            ...
                            <input type="hidden" name="tax_amount" id="tax-amount-input" value="{{ $purchase->tax_amount }}">

                            <div class="form-group">
                                <label>Subtotal</label>
                                <p class="font-weight-bold">Rp <span id="subtotal-amount">{{ number_format($purchase->total, 2, ',', '.') }}</span></p>
                            </div>
                            <div class="form-group">
                                <label>Pajak</label>
                                <p class="font-weight-bold">Rp <span id="tax-amount-display">{{ number_format($purchase->tax_amount, 2, ',', '.') }}</span></p>
                            </div>
                            <div class="form-group">
                                <label>Grand Total</label>
                                <p class="font-weight-bold text-success" style="font-size: 1.2em;">Rp <span id="grand-total">{{ number_format($purchase->total + $purchase->tax_amount, 2, ',', '.') }}</span></p>
                            </div>
